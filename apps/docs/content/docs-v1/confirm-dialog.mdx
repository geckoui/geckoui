...
> If you declare `onConfirm` as an `async` function, the dialog shows a loading state on the confirm button until the promise resolves. In v1 `onCancel` is not awaited, so it never shows one.
...
When `onConfirm` is declared `async`, the dialog automatically:
- Awaits the Promise
- Shows loading spinner on the button
- Closes after completion

The check looks at the function declaration, not at what it returns, so a function that
returns a promise without the `async` keyword gets no loading state.
...
- [Dialog](/v1/docs/dialog) - Base modal component
- [Alert](/v1/docs/alert) - Non-blocking notifications
- [LoadingButton](/v1/docs/loading-button) - Buttons with loading states
