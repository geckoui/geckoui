...
Custom `secondary` and `danger` colors created using module augmentation:
...
Add the corresponding styles to your global CSS file. Target using `data-variant` and `data-color` attribute selectors:
...
.GeckoUIButton[data-variant="filled"][data-color="danger"] {
...
- `data-variant` - Variant (filled, outlined, ghost, icon)
- `data-color` - Color (primary, or custom via module augmentation)
- `data-size` - Size (xs, sm, md, lg, xl)

Target them in CSS with attribute selectors:
...
- [LoadingButton](/v1/docs/loading-button) - Button with loading state
