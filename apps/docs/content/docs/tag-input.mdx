...
The placeholder is drawn as text rather than the input's own `placeholder`, because the text box is only ever as wide as what is in it — a native placeholder would be clipped to a couple of pixels.
