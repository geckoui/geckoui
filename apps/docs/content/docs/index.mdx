...
### 3. Add GeckoUIProvider

Wrap your app with `GeckoUIProvider`. It is required for Toast, ConfirmDialog and the imperative `Dialog.show()` / `Drawer.show()`; the declarative `<Dialog open>` and `<Drawer open>` work without it:
