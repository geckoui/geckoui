...
| `indeterminate` | `boolean \| Function` | `false` | Show indeterminate state |
...
Use the `indeterminate` prop to show an indeterminate state (useful for "select all" scenarios):
...
// Dynamic indeterminate based on field state
<RHFCheckbox
  name="items"
  value="item1"
  indeterminate={({ field }) => field.value.length > 0 && field.value.length < total}
/>
```
