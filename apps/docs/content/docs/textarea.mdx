...
- It grows as you type and shrinks again when text is removed
- `rows` is the shortest it gets, `maxRows` the tallest, after which it scrolls
- It follows whatever font, size, line height and padding you give it, and refits when a web font finishes loading
- **The drag handle is off**, since the height is the component's — a manual resize would be undone by the next keystroke

Put the handle back if you would rather have it, and it will fight the next keystroke:
