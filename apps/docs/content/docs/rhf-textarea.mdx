...
| `transform` | `{ input?: (v: string) => string; output?: (v: string) => string }` | - | Format the value for display and for storage |
...
- `aria-invalid` on the control when `fieldState.error` exists, which both a screen reader and the red border read
