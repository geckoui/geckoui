...
A custom `critical` variant created using module augmentation:
...
The component uses `data-*` attributes for state and variant styling:

- `data-variant` — Variant (error, warning, info, success, default)
...
| `--gecko-alert-accent` | per variant | Title text and icon |
| `--gecko-alert-radius` | `0.5rem` | Corner radius |

Scope a variable to one variant to restyle only that one:
