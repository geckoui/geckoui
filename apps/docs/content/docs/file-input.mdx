...
import { FileInputBasicExample, FileInputMultipleExample, FileInputAppendExample, FileInputRejectExample, FileInputStatesExample, FileInputRenderExample, FileInputAvatarExample } from '@/components/examples/file-input-examples';
  ...
  render={({ files, dragging }) => {
    ...
    return (
      <span
        ...
        {picked ? (
          <img src={picked.preview} alt={picked.name} className="h-full w-full object-cover" />
        ) : (
...
No `multiple`, so `files` holds at most one and dropping a second image replaces the first
rather than adding to it. `preview` gives it the object URL to draw, revoked for you when
