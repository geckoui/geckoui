...
| `offset` | `number` | `6` | How far the panel sits from the trigger, in pixels |
...
panel itself when there is nothing — and returns to the trigger when it closes. Without
that, a keyboard user opens a popover and is left behind it.
