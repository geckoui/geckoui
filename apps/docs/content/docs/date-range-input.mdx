...
| `fixedWeeks` | `boolean` | `false` | Always render six weeks in the calendar, so the popup keeps the same height across months |
...
```tsx
<DateRangeInput value={value} onChange={setValue} aria-invalid />
```
...
- `aria-invalid` - The field is invalid, set by you or by a date that cannot exist
...
.GeckoUIDateInput[aria-invalid="true"] { border-color: var(--color-border-invalid); }
