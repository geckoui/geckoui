...
A file input wired to React Hook Form, creating preview URLs for whatever is selected. Single or multiple, with custom rendering.

**This one or [RHFFilePicker](/docs/rhf-file-picker)?** Use `RHFFileInput` for a plain click-to-browse field, and when the value should be one file rather than a list. Use `RHFFilePicker` when you want drag and drop, directory selection, or duplicate detection — it always holds an array.
