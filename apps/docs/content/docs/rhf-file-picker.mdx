...
A file picker wired to React Hook Form, with drag and drop, directory selection and duplicate detection. The field holds a `FilePickerFile[]`.

**This one or [RHFFileInput](/docs/rhf-file-input)?** Use `RHFFilePicker` when you want drag and drop, directory selection, or duplicate detection. Use `RHFFileInput` for a plain click-to-browse field, or when the value should be one file rather than a list.

Pass `multiple={false}` to keep a single file while still accepting a drop.
...
| `multiple` | `boolean` | `true` | Whether more than one file may be held. `false` keeps one, browsed or dropped |
...
| `removeDuplicates` | `boolean` | `false` | Drop a file already held, or one that arrived twice in the same batch |
...
| `render` | `(props) => ReactNode` | - | Draw it yourself. Given `{ dropzoneRef, dragging, loading, openFilePicker, files, disabled }` plus the RHF render args |
...
`removeDuplicates` drops a file that is already held, and one that arrives twice in the same
batch — dropping a folder and a subfolder that both contain it, say. It works with or without
`keepOldFiles`.

Files are compared by size first, then by sampling the start, middle and end, so a large file
is never read end to end.
